...
// parse the AST or do a robust replace
for (const [slug, data] of Object.entries(enrichments)) {
  ...
  // Use a string replace that reliably targets the line before "pipeline":
  const searchPattern = new RegExp('("slug": "' + slug + '",[\\\\s\\\\S]*?)(?="pipeline":)');
...
console.log("Competitors fixed successfully!");
