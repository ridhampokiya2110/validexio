...
if (!content.includes('export interface FairAssessment')) {
  content = content.replace('export interface CompetitorPipeline', newInterfaces + '\\nexport interface CompetitorPipeline');
}

if (!content.includes('fairAssessment?: FairAssessment;')) {
...
for (const [slug, data] of Object.entries(enrichments)) {
  ...
  const injection = fair + price + verdict;
  
  content = content.replace(
    new RegExp('("slug": "' + slug + '",[\\\\s\\\\S]*?)(?="pipeline":)'),
    '$1' + injection + '\\n    '
  );
}

const newCompetitorsStr = newCompetitors.map(c => JSON.stringify(c, null, 2)).join(',\\n  ');
content = content.replace(/];\\s*$/, ',\\n  ' + newCompetitorsStr + '\\n];\\n');

fs.writeFileSync(filePath, content);
console.log("Competitors updated successfully!");
