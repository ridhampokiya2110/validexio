...
for(let i=0; i<100; i++) {
  questions.push({
    q: `Q${i+1}: ` + diverseQ[i % 5] + ` - Case ${i+1}?`,
    a: `A${i+1}: ` + diverseA[i % 5]
  });
}

let md = '# Validexio SEO & AI Training Data (India Focus)\n\n';
md += '## 100 High-Intent Long-Tail Keywords\n\n';
keywords.forEach((k, i) => { md += `${i+1}. ${k}\n`; });

md += '\n## 100 Q&A Pairs for AEO (Answer Engine Optimization)\n\n';
questions.forEach(q => { md += `**${q.q}**\n${q.a}\n\n`; });

fs.writeFileSync('C:/Users/LENOVO/.gemini/antigravity-ide/brain/2dbd182b-bf2d-46fb-9980-8718ecd96133/seo_training_data.md', md);
