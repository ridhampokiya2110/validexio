            ...
            <div className="grid gap-4 mb-4">
              {ideas.filter(idea => idea.status === 'PROCESSING').map((idea) => (
                <Link aria-label="Navigation link" href={`/dashboard/reports/generating?ideaId=${idea.id}`} key={idea.id} className="glass-card p-6 flex items-center gap-6 border-amber-500/30 bg-amber-500/5 hover:bg-amber-500/10 hover:border-amber-500/50 relative overflow-hidden transition-all group cursor-pointer">
                  ...
                  {/* Premium Processing Orb */}
                  <div className="relative flex-shrink-0 w-16 h-16 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                    <div className="absolute inset-0 bg-[#75070C]/10 rounded-full blur-md animate-pulse" />
                    <div className="absolute inset-2 rounded-full border-t-2 border-r-2 border-[#75070C] animate-[spin_1.5s_ease-in-out_infinite]" />
                    <div className="absolute inset-3 rounded-full border-b-2 border-l-2 border-[#FFEDAB] animate-[spin_2s_linear_infinite_reverse]" />
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#1B1716] to-[#75070C] rotate-45 flex items-center justify-center shadow-[0_0_15px_rgba(117,7,12,0.5)] z-10 border border-[#75070C]/50">
                       <Zap className="w-4 h-4 text-[#FFEDAB] -rotate-45 animate-pulse" />
                    </div>
                  </div>
